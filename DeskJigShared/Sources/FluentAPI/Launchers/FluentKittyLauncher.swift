//  FluentKittyLauncher.swift
//  DeskJigShared

import Foundation
import AppKit

public struct FluentKittyLauncher: FluentAppLauncher {
    public let bundleId = "net.kovidgoyal.kitty"
    public let appName = "kitty"
    ...
    public func launch(
        ...
        // Get the full kitty app path
        let kittyAppPath = NSWorkspace.shared.urlForApplication(
            withBundleIdentifier: bundleId
        )?.path ?? "/Applications/kitty.app"

        // Build arguments: open -na <app> --args --single-instance --directory=<path> --title=<title>
        var args = ["-na", kittyAppPath, "--args", "--single-instance"]
        if let dir = directory {
            args.append("--directory=\(dir)")
        }
        ...
        do {
            ...
            DeskJigLog.debug(.restorationTrace, "kitty launch \(exitCode == 0 ? "completed" : "failed")", fields: [
                "taskId": task.taskId,
                "taskType": task.taskType.rawValue,
            ...
            let durationMs = Int(Date().timeIntervalSince(startTime) * 1000)
            DeskJigLog.debug(.restorationTrace, "kitty launch error: \(error.localizedDescription)", fields: [
                "taskId": task.taskId,
                "taskType": task.taskType.rawValue,
                "durationMs": "\(durationMs)"
            ], runId: task.runId)
    ...
    /// Launches kitty attached to a tmux session on the DeskJig socket.
    public func launchWithTmuxSession(
        ...
        if let idx = tmuxManagedIndex {
            managedTitle = BundleRegistry.managedTmuxWindowTitle(bundleId: BundleRegistry.kitty, index: idx)
        } else {
            managedTitle = BundleRegistry.managedTmuxWindowTitle
        }

        // open -na kitty --args --single-instance --title=<title> tmux -S <socket> new-session -A -s <name> -c <dir>
        var args = ["-na", kittyAppPath, "--args", "--single-instance"]
        args.append("--title=\(managedTitle)")
        ...
        do {
            ...
            DeskJigLog.debug(.restorationTrace, "kitty tmux launch \((exitCode == 0 && verification.success) ? "completed" : "failed")", fields: [
                "taskId": task.taskId,
                "taskType": task.taskType.rawValue,
                "durationMs": "\(durationMs)",
                "exitCode": "\(exitCode)",
    ...
    public func matchWindow(
        ...
        let kittyWindows = snapshot.windows(forBundleID: bundleId)

        guard !kittyWindows.isEmpty else {
            DeskJigLog.debug(.restorationTrace, "No kitty windows found", fields: [
                "taskId": task.taskId,
                "taskType": task.taskType.rawValue
            ], runId: task.runId)
            return nil
        ...
        if let expectedTitle = title, expectedTitle.hasPrefix("\(BundleIdentity.terminalTitleTokenPrefix):") {
            let matchingTitleWindows = kittyWindows.filter { $0.title == expectedTitle }
            ...
            let containsTitleWindows = kittyWindows.filter {
            ...
            if trustDeskJigTitle, let dir = directory {
                let normalizedDir = Self.normalizePath(dir)
                let dirMatches = kittyWindows.filter {
                    guard let workingDir = $0.freshWorkingDirectory ?? $0.documentPath else { return false }
                    return Self.normalizePath(workingDir) == normalizedDir
                }
            ...
            DeskJigLog.debug(.restorationTrace, "No kitty window with legacy title token '\(expectedTitle)' found", fields: [
                "taskId": task.taskId,
                "taskType": task.taskType.rawValue
            ], runId: task.runId)
            return nil
        ...
        if let dir = directory {
            let normalizedDir = Self.normalizePath(dir)
            if let window = kittyWindows.first(where: {
                guard let workingDir = $0.freshWorkingDirectory ?? $0.documentPath else { return false }
                return Self.normalizePath(workingDir) == normalizedDir
            }) {
                let matchedPath = window.freshWorkingDirectory ?? window.documentPath ?? dir
            ...
            // Strategy 3: Title pattern match (kitty often includes path in title)
        ...
        // Strategy 4: Return any kitty window (lowest confidence)
        if let window = kittyWindows.first {
            DeskJigLog.debug(.restorationTrace, "Matched \(appName) window", fields: [
                "taskId": task.taskId,
                "taskType": task.taskType.rawValue,
                "app": appName,
