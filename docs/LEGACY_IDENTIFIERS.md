...
| ~~`/tmp/bento-tmux-<uid>.sock`~~ | tmux command service | **Renamed to `/tmp/deskjig-tmux-<uid>.sock` (2026-08-25, maintainer decision).** Sessions on the old socket become unmanaged after upgrade and are recreated on the next workspace restore — equivalent to a one-time Reset Sessions. Session name prefixes (`bento_…`) and managed pane titles are unchanged. |
