...
struct SettingsScreen: View {
    ...
    private var showTmux: Bool {
        // Hidden: see `showDeskJigCLI`. Kept intact pending a new home in settings.
        false
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            // Terminal (tmux) Section
            if showTmux {
                SettingsSection("Terminal (tmux)", badgeText: "Beta") {
                    VStack(alignment: .leading, spacing: 16) {
                        SettingsToggle(
...
extension SettingsScreen {
    ...
    private func loadTmuxInfo() {
        Task.detached(priority: .utility) {
            DeskJigLog.info(.app, "[SettingsScreen] Starting tmux status refresh")
            ...
            DeskJigLog.info(.app, "[SettingsScreen] Finished tmux status refresh", fields: ["installed": "\(installed)", "sessions": "\(sessionCount)"])
        }
    }
