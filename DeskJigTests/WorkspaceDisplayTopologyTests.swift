...
struct WorkspaceDisplayTopologyTests {
    ...
    @Test("URL handoff screen geometry resolves an upper stacked display using the primary anchor")
    func urlHandoffScreenGeometryUsesPrimaryDisplayAnchor() {
        let primary = makeScreen(
            displayID: 4,
            name: "Primary",
            origin: CGPoint(x: 0, y: 0),
        ...
        let screens = [upper, primary]
        let referenceFrame = CGRect(x: 100, y: -900, width: 900, height: 700)

        let preferredIndex = URLHandoffScreenGeometry.preferredScreenIndex(
            referenceFrame: referenceFrame,
            screens: screens
        )
        let visibleFrame = URLHandoffScreenGeometry.screenVisibleFrame(
            for: preferredIndex,
            screens: screens
        )

        #expect(preferredIndex == 1)
        #expect(visibleFrame == CGRect(x: 0, y: -1050, width: 1920, height: 1050))
    }
